...
class SchemaValidator:
    ...
    def get_required_schemas(self, repository_types: List[str]) -> List[str]:
        ...
        for repo_type in repository_types:
            if repo_type in type_schemas:
                ...
                all_required.update([schema.replace('.yaml', '') for schema in required])
    ...
    def get_optional_schemas(self, repository_types: List[str]) -> List[str]:
        ...
        for repo_type in repository_types:
            if repo_type in type_schemas:
                ...
                all_optional.update([schema.replace('.yaml', '') for schema in optional])
    ...
    def _create_error_result(self, repo_path: Path, errors: List[str]) -> Dict[str, Any]:
    ...
    def _validate_required_schemas(self, omd_dir: Path, results: Dict[str, Any], 
                                 required_schemas: List[str], repo_type: List[str]):
        ...
        for schema_name in required_schemas:
            ...
            if config_file.exists():
                self._validate_and_record_schema(config_file, schema_name, results, is_required=True)
            else:
                results["valid"] = False
    ...
    def _validate_optional_schemas(self, omd_dir: Path, results: Dict[str, Any], 
                                 optional_schemas: List[str]):
        """Validate optional schemas if they exist."""
        for schema_name in optional_schemas:
            config_file = omd_dir / f"{schema_name}.yaml"
            if config_file.exists():
                self._validate_and_record_schema(config_file, schema_name, results, is_required=False)

    def _validate_and_record_schema(self, config_file: Path, schema_name: str, 
                                  results: Dict[str, Any], is_required: bool):
